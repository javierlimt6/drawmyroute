...
from app.services.shape_service import generate_route, generate_route_with_bounds, load_shapes
...
async def generate_route_endpoint(request: RouteRequest):
    ...
    try:
        # If target_bounds is provided, use bounds-based routing (authoritative box)
        if request.target_bounds:
            return await generate_route_with_bounds(
                shape_id=request.shape_id,
                prompt=request.prompt,
                text=request.text,
                image_svg_path=request.image_svg_path,
                min_lat=request.target_bounds.min_lat,
                max_lat=request.target_bounds.max_lat,
                min_lng=request.target_bounds.min_lng,
                max_lng=request.target_bounds.max_lng,
                fast_mode=request.fast_mode
            )
        
        # Otherwise, use distance-based routing with iterative scaling
